...
data0 = pd.read_excel('training set good.xlsx')

data0.iloc[:, numb] = data0.iloc[:, numb].apply(lambda x: 0 if x < 90 else x)
data0.to_excel('training set good.xlsx',index=False)
...
# 使用模型预测新的学生数据
data1 = pd.read_excel('stu set good.xlsx')
...
# graph.write_png('D:/大学文件/计算机编程实验与网页设计实验/python编程实验/08 大创项目算法/pythonProject1/学生端/课程成绩预测/pictures/优秀预测.png')
